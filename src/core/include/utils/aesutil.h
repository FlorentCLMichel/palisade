...
class AESUtil {
 private:
  ...
  int KeySchedule(unsigned int rk[], unsigned int rrk[],
                  const unsigned char cipherKey[], int keyBytes);
  int KeyExpansion(unsigned int rk[], const unsigned char cipherKey[],
                   int keyBytes);
 ...
 public:
  ...
  void Encrypt(unsigned char *pData, unsigned char *pCipher,
               unsigned int DataLen, ModeOfOperation mode);
  void Decrypt(unsigned char *pCipher, unsigned char *pData,
               unsigned int CipherLen, ModeOfOperation mode);
