...
#define GETU32(pt)                                                 \
  (((unsigned int)(pt)[0] << 24) ^ ((unsigned int)(pt)[1] << 16) ^ \
   ((unsigned int)(pt)[2] << 8) ^ ((unsigned int)(pt)[3]))  // NOLINT
...
int AESUtil::KeyExpansion(unsigned int rk[], const unsigned char cipherKey[],
                          int keyBytes) {
  int i = 0;
  unsigned int temp;
...
int AESUtil::KeySchedule(unsigned int rk[], unsigned int rrk[],
...
void AESUtil::Encrypt(unsigned char *pData, unsigned char *pCipher,
  ...
  if (mode == CTR) {
    ...
    for (i = 0; i < DataLen; i += BLOCK_SIZE) {
      ...
      pCipher += BLOCK_SIZE;
      for (k = 16; k > 0; k--)
        if (++counterblock[k - 1] != 0) break;
    }
  }
}

void AESUtil::Decrypt(unsigned char *pCipher, unsigned char *pData,
